...
export default{
   ...
   mounted(){
        ...
        this.id_editorial = route.params.id;
        this.getEditorial();
   },
   methods:{
        ...
        getEditorial() {
            axios.get('http://localhost:8000/api/editorials/'+this.id_editorial).then(
                res => (
                    this.name = res.data.results.name_editorial
                )
            )
        },
       ...
       updateEditorial() {
           ...
           var url = 'http://localhost:8000/api/editorials/'+this.id_editorial;
           ...
           axios({method:'PUT', url:url, data:parametros}).then(function(respuesta){
                   ...
                   if (status == 'success') {
                       show_alerta('Editorial actualizado correctamente', status);
